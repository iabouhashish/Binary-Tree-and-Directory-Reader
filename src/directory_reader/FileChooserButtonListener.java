...
public class FileChooserButtonListener implements ActionListener {
	...
	private static void buildTree(File file, FileNode curr) {
		...
		for (int i = 0; i < list.length; i++) {
		      if (list[i].isFile()) {
		    	FileNode child = new FileNode(list[i].getName().toString(), curr, FileType.FILE);
		        curr.addChild("child " + i, child);
		      } else if (list[i].isDirectory()) {
		    	  FileNode child = new FileNode(list[i].getName().toString(), curr, FileType.DIRECTORY);
			      curr.addChild("child " + i, child);
	...
	private static void buildDirectoryContents(FileNode fileNode, StringBuffer contents, String prefix) {
		...
		Iterator<FileNode> iterator = fileNode.getChildren().iterator();
		while (iterator.hasNext()) {
			FileNode current = iterator.next();
			if (current.isDirectory()) {
				contents.append("\n");
				buildDirectoryContents(current, contents, prefix);
			} else {
				contents.append("\n");
				contents.append(prefix + current.getName());
			}
